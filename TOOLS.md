# List of Tools

## General
- [Mitre Attack Matrix](https://attack.mitre.org/).
- [SecLists](https://github.com/danielmiessler/SecLists).
- [HaveIBeenPwned]().
- [termbin.com]().

---

## Compilations
- [OWASP Projects](https://owasp.org/projects/).

---

## OPSEC
- Protectli pfSense Firewall.
- Silent Pocket Faraday Bag.
- Yubikey.
- [Microphone Lock](https://www.amazon.com/Mic-Lock-Hacking-Computers-Tablets-blocker/dp/B076CSLB5H).
- [Slate Portable Router](https://www.amazon.com/GL-iNet-GL-AR750S-Ext-pre-Installed-Cloudflare-Included/dp/B07GBXMBQF).

### Email

- Tutanota.
- ProtonMail.
- HushMail.

---

## Reconnaissance

### General
- [Recon-ng](https://github.com/lanmaster53/recon-ng).
- [Sn1per](https://github.com/1N3/Sn1per). Automated pentest framework for offensive security (including recon).
- [Pentest Tools](https://pentest-tools.com/home). Free basic, web-based pentest tools.
- [OSINT Framework]().
- [OSINT.link]().
- [inteltechniques.net]().
- [Hunchly]().

### Emails
- [Hunter.io]().
- [theHarvester]().

### History
- [Wayback Robots](https://gist.github.com/mhmdiaa/2742c5e147d49a804b408bfed3d32d07).
- [Wayback URLs](https://gist.github.com/mhmdiaa/adf6bff70142e5091792841d4b372050).
- [WaybackPy](https://github.com/akamhy/waybackpy).

### Google Dorks
- [Basics](https://support.google.com/websearch/answer/2466433?hl=en). Official help from Google. ([archive](https://archive.is/JiAij))
- [Pentest Tools Google Hacking](https://pentest-tools.com/information-gathering/google-hacking).
- [`1N3/Goohak`](https://github.com/1N3/Goohak). Automatically launches Google hacking queries against a target domain.

### AWS
- [Sandcastle](https://github.com/0xSearches/sandcastle).
- [Bucket Finder](https://digi.ninja/projects/bucket_finder.php).

### Source code
- [Gitrob](https://github.com/michenriksen/gitrob).
- [truffleHog](https://github.com/dxa4481/truffleHog). Search through git repositories for high entropy strings and secrets.
- [git-all-secrets](https://github.com/anshumanbh/git-all-secrets). Captures git secrets by leveraging multiple open source git searching tools.
- [RepoSessed](https://github.com/IOActive/RepoSsessed). Searches for vulnerabilities in Git repositories.

### Devices
- [Shodan](https://www.shodan.io/).
- [Censys.io](https://censys.io/).

### Context gathering
- Desktop OS, Browser Statistics (Market Share)
  - [StatCounter GlobalStats](https://gs.statcounter.com/macos-version-market-share/desktop/worldwide)

### Domains
- [`dig`]().
- [`WHOIS`]().
- [`nslookup`]().
- [`dnsrecon`]().
- [Bluto]().
- [crt.sh](https://crt.sh/).
- [viewdns.info](https://viewdns.info/reversewhois/?q=).
- [dnsdumpster](https://dnsdumpster.com/).
- [dnsgoodies.com](http://dnsgoodies.com/).
- [Sublist3r](https://github.com/aboul3la/Sublist3r). Fast subdomains enumeration tool for penetration testers.
- [`dnsscan`](https://github.com/rbsec/dnscan). DNS subdomain scanner.
- [Aquatone](https://github.com/michenriksen/aquatone). Domain screenshots.
- Virtual Hosts
  - [virtual-host-scanner](https://github.com/jobertabma/virtual-host-discovery).
  - [Pentest Tools, Virtual Host Scanner](https://pentest-tools.com/information-gathering/find-virtual-hosts).

### Websites
- [relative-url-extractor](https://github.com/jobertabma/relative-url-extractor).
- [httpscreenshot](https://github.com/breenmachine/httpscreenshot/). Gathers screenshots of a large number of websites.
- [EyeWitness](https://github.com/FortyNorthSecurity/EyeWitness). Take screenshots of websites, provide header info, and identify default credentials if possible.
- [wappalyzer]().
- [WhatWeb]().
- [BuiltWith]().
- [Security Headers](https://securityheaders.com).
- [SSL Server Test](https://ssllabs.com/ssltest/).

### Security
- Firewalls
  - [WAFW00F](https://github.com/EnableSecurity/wafw00f). Web application firewall fingerprinting tool.
- Web filtering
  - [Palo Alto Network Filter](https://urlfiltering.paloaltonetworks.com/).
  - [McAfee Site Checker](https://www.trustedsource.org/en/feedback/url?action=checksingle).
  - [Symantec WebPulse](https://sitereview.bluecoat.com/#/).
  - [Forcepoint Site Lookup Tool](https://www.websense.com/content/Sitelookup.aspx).
  - [Barracuda IP/Domain Lookup](https://www.barracudacentral.org/lookups).
  - [Fortiguard Web Filter](https://www.fortiguard.com/webfilter).

### Network Scanning
- [Masscan](https://github.com/robertdavidgraham/masscan).
- [Zmap](https://github.com/zmap/zmap).
- [Xray](https://github.com/evilsocket/xray).
- [Nmap](https://nmap.org/).

### Social media
- [SimplyEmail](https://github.com/SimplySecurity/SimplyEmail).
- [`nettitude/prowl`](https://github.com/nettitude/prowl).
- `tweets_analyzer.py`.

---

## Phishing and initial access
- [wep](https://github.com/ghost123gg/wep). Generate VBA macros.
- [Invoke-Obfuscation](https://github.com/danielbohannon/Invoke-Obfuscation). Obfuscate PowerShell commands.

---

## AWS
- [`awspx`](https://labs.f-secure.com/tools/awspx/).

---

## Proxies
- `iptables`
- `proxychains`
- [**Modlishka**](https://github.com/drk1wi/Modlishka). Advanced reverse proxy. Can also be used for phishing.

---

## Hosting
- [**file.io**](https://file.io). Free temporary file hosting.

---

## Mobile
- [`apktool`](https://ibotpeaches.github.io/Apktool/).
- [MobSF](https://github.com/MobSF/Mobile-Security-Framework-MobSF).

---

## Web
- [Burp Suite](https://portswigger.net/burp).

---

## Vulnerability Scanning
- [FuzzDB](https://github.com/fuzzdb-project/fuzzdb).
- [Atomic Red Team](https://github.com/redcanaryco/atomic-red-team).

### Web
- [Nikto](https://cirt.net/Nikto2).
- [SQLmap](https://github.com/sqlmapproject/sqlmap).
- [`nettitude/xss_payloads`](https://github.com/nettitude/xss_payloads).

### Linux
- [Linux Exploit Suggester](https://github.com/mzet-/linux-exploit-suggester).

---

## Hardware
- Badge cloning
  - Boscloner Pro

---

## Reverse Engineering
- [radare2](https://www.radare.org/r/).
- [IDA](https://www.hex-rays.com/products/ida/).


### Windows
- [Resource Hacker](http://www.angusj.com/resourcehacker/).

---

## Exploitation
- [American Fuzzy Lop](https://lcamtuf.coredump.cx/afl/).
- Radamsa.
- Peach Fuzzer.
- BooFuzz.
- BFuzz.
- [Metasploit](https://www.metasploit.com/).

### C2
- [PostC2](https://github.com/nettitude/PoshC2).
- [Mythic](https://github.com/its-a-feature/Mythic). Previously known as Apfell.

### Malware
- [VirusTotal](https://www.virustotal.com/gui/home/upload).


---

## Networks
- [Wireshark](https://www.wireshark.org/).
- [Aircrack-ng](https://github.com/aircrack-ng/aircrack-ng).

---

## Credentials
- [Hashcat](https://hashcat.net/hashcat/).
- [John the Ripper](https://www.openwall.com/john/).
- [Hydra](https://github.com/vanhauser-thc/thc-hydra).

---

## Forensics
- [Catfish](http://www.twotoasts.de/index.php/catfish/).


### Office
- `olevba`

### Social Media
- `tweets_analyzer.py`.

---

## Anti-forensics
- [Secure RM](https://sourceforge.net/projects/srm/).

---

## Situational Awareness
- `netdiscover`

### Windows
- [JAWS](https://github.com/411Hall/JAWS). Windows enumeration script written in PowerShell.

---

## Windows

### Tools
- [`FuzzySecurity/PowerShell-Suite`](https://github.com/FuzzySecurity/PowerShell-Suite).

---

## macOS

### Development
- [No Overview Available](https://nooverviewavailable.com). A survey of Apple documentation coverage.
- [NSHipster](https://nshipster.com). A journal of the overlooked aspects in Objective-C, Swift, and Cocoa.

---

## Utilities
- `xxd`

---

## References
- TOOLS1
